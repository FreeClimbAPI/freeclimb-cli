  ...
  "message": "Response from server"
    ...
    describe("recordings:get next flag test", function () {
        ...
        test.nock("https://www.freeclimb.com", async (api) =>
            api
                .get(`/apiserver/Accounts/${await cred.accountId}/Recordings/${recordingId}`)
                .query({ cursor: "7265636f7264696e67733a676574" })
                .basicAuth({ user: await cred.accountId, pass: await cred.authToken })
                .reply(200, testJsonNext)
        )
            .stdout()
            .env({ FREECLIMB_RECORDINGS_GET_NEXT: "7265636f7264696e67733a676574" })
            .command(["recordings:get", "userInput-recordingId", "--next"])
            .it(
        ...
        before(() => {
            ;(async () => {
                testJsonNext2.nextPageUri = `https://www.freeclimb.com/apiserver/Accounts/${await cred.accountId}/Recordings/${recordingId}?cursor=${finalCursor}`
            })()
        })
        after(() => {
            const dataDir = data.run([]).then(
                (dataDir) => {
                    ...
                    environment.clearString("FREECLIMB_RECORDINGS_GET_NEXT")
                },
