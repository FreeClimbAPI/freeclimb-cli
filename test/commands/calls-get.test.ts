...
describe("calls:get Data Test", function () {
  ...
  "message": "Response from server"
    ...
    describe("calls:get next flag test", function () {
        ...
        before(() => {
            ;(async () => {
                testJsonNext2.nextPageUri = `https://www.freeclimb.com/apiserver/Accounts/${await cred.accountId}/Calls/${callId}?cursor=${finalCursor}`
            })()
        })
        after(() => {
            const dataDir = data.run([]).then(
                (dataDir) => {
                    ...
                    environment.clearString("FREECLIMB_CALLS_GET_NEXT")
                },
