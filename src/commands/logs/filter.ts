...
export class logsFilter extends Command {
	...
	static args = [		{name: "pql", description: "The filter query for retrieving logs. See Performance Query Language below.", required: true},
]
    ...
    async run() {
        ...
        const nextResponse = (response: FreeClimbResponse) => {if (response.data) { out.out(flags.maxItem?JSON.stringify(response.data.logs.splice(0, flags.maxItem), null, 2): JSON.stringify(response.data, null, 2)) } else { throw new Errors.UndefinedResponseError()}
            if(out.next === null) {
